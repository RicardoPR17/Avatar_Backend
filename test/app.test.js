...
describe("GET db/user/:email", () => {
    ...
    test("should respond with an array", async () => {
        const response = await request(app).get("/db/user/laura.garcia-a_mail.escuelaing.edu.co#EXT#@avatarnavi.onmicrosoft.com");
        expect(response.body).toBeInstanceOf(Array)
    });
...
describe ("GET db/user/:email/balance", () => {
    ...
    test("should verify the user's balance", async () => {
        const response = await request(app).get("/db/user/jessica.munoz-o_mail.escuelaing.edu.co#EXT#@avatarnavi.onmicrosoft.com/balance");
